...
def check_alerts():
    ...
    if snapshot["total_requests"] < settings.MONITORING_MIN_REQUESTS_FOR_ALERT:
        return

    if snapshot["error_rate"] >= settings.MONITORING_ERROR_RATE_THRESHOLD:
        _emit_alert(
    ...
    if snapshot["p95_latency_ms"] >= settings.MONITORING_LATENCY_P95_THRESHOLD_MS:
        _emit_alert(
